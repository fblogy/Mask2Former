...
from .modeling.criterion_selectquery import SetCriterion_SelectQuery
from .modeling.matcher_selectquery import HungarianMatcher, HungarianMatcher_Encoder
import time

@META_ARCH_REGISTRY.register()
class MaskFormerSelectQuery(nn.Module):
    """
    ...
    def from_config(cls, cfg):
        ...
        matcher_encoder = HungarianMatcher_Encoder(
            cost_class=class_weight,
            cost_mask=mask_weight,
            cost_dice=dice_weight,
            num_points=cfg.MODEL.MASK_FORMER.TRAIN_NUM_POINTS,
        ...
        weight_dict = {"loss_ce": class_weight, "loss_encoder_ce": class_weight, "loss_mask": mask_weight, "loss_dice": dice_weight}

        if deep_supervision:
            dec_layers = cfg.MODEL.MASK_FORMER.DEC_LAYERS
            aux_weight_dict = {}
            for i in range(dec_layers - 1):
                aux_weight_dict.update({k + f"_{i}": v for k, v in weight_dict.items() if k != "loss_encoder_ce"})
            weight_dict.update(aux_weight_dict)

        losses = ["labels", "encoderlabels", "masks"]

        criterion = SetCriterion_SelectQuery(
            sem_seg_head.num_classes,
            matcher=matcher,
            matcher_encoder=matcher_encoder,
