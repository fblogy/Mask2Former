...
class REFER:
    ...
    def getAnnIds(self, image_ids=[], cat_ids=[], ref_ids=[]):
        ...
        if len(image_ids) == len(cat_ids) == len(ref_ids) == 0:
            ...
            if not len(image_ids) == 0:
                ...
                anns = []
            if not len(cat_ids) == 0:
                anns.extend([ann for ann in anns if ann['category_id'] in cat_ids])
            ann_ids = [ann['id'] for ann in anns]

            if not len(ref_ids) == 0:
                ann_ids.extend([self.Refs[ref_id]['ann_id'] for ref_id in ref_ids])
    ...
    def getImgIds(self, ref_ids=[]):
        ...
        if not len(ref_ids) == 0:
            image_ids = [self.Refs[ref_id]['image_id'] for ref_id in ref_ids]
        else:
    ...
    def getRefBox(self, ref_id):
        # NOTE: Another extraction of referring object.
