...
class HungarianMatcher(nn.Module):
    ...
    def memory_efficient_forward(self, outputs, targets):
        ...
        for b in range(bs):
            ...
            out_decouple_prob = outputs["pred_decouple_logits"][b].softmax(-1)
            ...
            cost_decouple_class = -out_decouple_prob[:, tgt_ids]
            ...
            # Final cost matrix
            C = (self.cost_class * cost_class + self.cost_dice * cost_dice + self.cost_mask * cost_mask)
            C_decouple = (cost_decouple_class)
            C = C.reshape(num_queries, -1).cpu()
            C_decouple = C_decouple.reshape(num_queries, -1).cpu()
            ...
            # match_q = match_result[0].tolist()C
            indices.append(match_result)
            decouple_match_result = linear_sum_assignment(C_decouple)
            decouple_indices.append(decouple_match_result)
        ...
        return [(torch.as_tensor(i, dtype=torch.int64), torch.as_tensor(j, dtype=torch.int64)) for i, j in indices], [(torch.as_tensor(i, dtype=torch.int64), torch.as_tensor(j, dtype=torch.int64)) for i, j in decouple_indices]
    @torch.no_grad()
