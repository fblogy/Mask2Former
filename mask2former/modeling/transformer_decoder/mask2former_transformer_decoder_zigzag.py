...
class MultiScaleZigZagTransformerDecoder(nn.Module):
    ...
    def forward(self, x, mask_features, mask=None):
        """Forawrd function describtion of MultiScaleTransformerDecoder
            x (torch.Tensor): multi scale features, pixel decoder multiple outputs.
            mask_features (torch.Tensor): pixel decoder output, shape like (B, C, H, W).
        """
    ...
    def zigzag(self, pixel_embeddings, mask, query_embedding):
        ...
        mask = mask > 0.5
        mask = mask.float()
        region_feats = torch.einsum('bchw,bqhw->qbc', pixel_embeddings, mask)
        region_feats = F.layer_norm(region_feats, [
            region_feats.shape[-1],
        ])

        query_embedding = F.layer_norm(query_embedding, [
            query_embedding.shape[-1],
        ])
        return region_feats + query_embedding
...
@TRANSFORMER_DECODER_REGISTRY.register()
class MultiScaleZigZagInTransformerDecoder(nn.Module):
