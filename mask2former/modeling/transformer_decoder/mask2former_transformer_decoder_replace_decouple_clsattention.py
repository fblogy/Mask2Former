...
class MultiScaleTransformerDecoderReplaceDecoupleClsAttention(nn.Module):
    ...
    def forward(self, x, mask_features, mask=None):
        ...
        predictions_class = []
        predictions_mask = []
        predictions_decouple_class = []
        # prediction heads on learnable query features
        outputs_class, outputs_mask, attn_mask, outputs_class_decouple = self.forward_prediction_heads(
            output, mask_features, attn_mask_target_size=size_list[0], output_class=output_class)
        predictions_class.append(outputs_class)
        predictions_mask.append(outputs_mask)
        predictions_decouple_class.append(outputs_class_decouple)
        ...
        out = {
            ...
            'pred_decouple_logits': predictions_decouple_class[-1],
            'aux_outputs': self._set_aux_loss(predictions_class if self.mask_classification else None, predictions_mask, predictions_decouple_class)
        }
        return out
    ...
    def forward_prediction_heads(self, output, mask_features, attn_mask_target_size, output_class):
        ...
        outputs_class_decouple = self.class_embed(decoder_output_class)

        mask_embed = self.mask_embed(decoder_output)
        outputs_mask = torch.einsum("bqc,bchw->bqhw", mask_embed, mask_features)

        return outputs_class, outputs_mask, None, outputs_class_decouple

    @torch.jit.unused
    def _set_aux_loss(self, outputs_class, outputs_seg_masks, outputs_decouple_class):
        # this is a workaround to make torchscript happy, as torchscript
        # doesn't support dictionary with non-homogeneous values, such
        # as a dict having both a Tensor and a list.
        if self.mask_classification:
            return [{"pred_logits": a, "pred_masks": b, "pred_decouple_logits": c} for a, b, c in zip(outputs_class[:-1], outputs_seg_masks[:-1], outputs_decouple_class[:-1])]
        else:
            return [{"pred_masks": b} for b in outputs_seg_masks[:-1]]


@TRANSFORMER_DECODER_REGISTRY.register()
class MultiScaleMaskedTransformerDecoderReplaceDecoupleClsAttention(MultiScaleTransformerDecoderReplaceDecoupleClsAttention):
    ...
    def forward_prediction_heads(self, output, mask_features, attn_mask_target_size, output_class):
        ...
        return outputs_class, outputs_mask, attn_mask, outputs_class_decouple
