...
class MultiScaleTransformerDecoderFocalDeNoising(nn.Module):
    ...
    def forward(self, x, mask_features, mask=None, targets=None):
        ...
        encoder_feature = self.decoder_norm(mask_features.flatten(2).permute(2, 0, 1)) #[HW,B,C]
        ...
        scores = encoder_output_class.sigmoid()[:, :, :-1].max(-1)[0] #flatten(1, 2)
        scores, indices = torch.topk(scores, k=self.num_queries, dim=1, sorted=True)
        # print(scores)
        # exit(0)
        ...
        if targets != None:
            ...
            for b in range(bs):
                ...
                for i in range(rt):
                    ...
                    out_mask = mask_features[b].float()[:, None]
                    # tgt_mask = target_masks[bs].float()
                    ...
                    DN_feat = torch.einsum("cp, np->ncp", out_mask, tgt_mask)
                    DN_feat = torch.mean(DN_feat, dim = 2)
...
@TRANSFORMER_DECODER_REGISTRY.register()
class MultiScaleMaskedTransformerDecoderFocalDeNoising(MultiScaleTransformerDecoderFocalDeNoising):
