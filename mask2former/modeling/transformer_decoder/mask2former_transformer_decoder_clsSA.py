...
class MultiScaleTransformerDecoderClsSA(nn.Module):
    ...
    def forward(self, x, mask_features, mask=None):
        ...
        for i in range(self.num_layers):
            ...
            #     print(output)
            if (i > 2):
                tgt_mask = torch.zeros((bs, output.shape[0], output.shape[0]), device=output.device)
                cls = outputs_class[:, :, :-1].argmax(-1) #[B, N]
                # print('cls', cls.shape)
                for b in range(cls.shape[0]):
                    for j in range(cls.shape[1]):
                        tgt_mask[b][j] = (cls[b] != cls[b, j])
                tgt_mask = tgt_mask.unsqueeze(1).repeat(1, self.num_heads, 1, 1).flatten(0, 1).detach()

                tgt_mask[torch.where(tgt_mask.sum(-1) == tgt_mask.shape[-1])] = False
                output = self.transformer_self_attention_layers[i](
                    output, tgt_mask=tgt_mask, tgt_key_padding_mask=None, query_pos=query_embed)
            else:
                output = self.transformer_self_attention_layers[i](
...
@TRANSFORMER_DECODER_REGISTRY.register()
class MultiScaleMaskedTransformerDecoderClsSA(MultiScaleTransformerDecoderClsSA):
