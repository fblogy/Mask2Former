...
import torch
from torch import align_tensors, nn, Tensor
...
class CrossAttentionLayer(nn.Module):
    ...
    def forward_post(self,
                     ...
                     query_pos: Optional[Tensor] = None):
        tgt2, att_weight = self.multihead_attn(
...
class MultiScaleTransformerDecoderAlignmaskAttentionSigmoid(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.alignweight = MLP(hidden_dim, hidden_dim, num_queries, 3)
    ...
    def forward(self, x, mask_features, mask=None):
        ...
        predictions_class = []
        predictions_mask = []
        predictions_alignmask = []
        # prediction heads on learnable query features
        outputs_class, outputs_mask, attn_mask, outputs_alignmask = self.forward_prediction_heads(
            output, mask_features, attn_mask_target_size=size_list[0], output_class=output_class)
        predictions_class.append(outputs_class)
        predictions_mask.append(outputs_mask)
        predictions_alignmask.append(outputs_alignmask)
        ...
        for i in range(self.num_layers):
            ...
            output_class, output_weight = self.transformer_cross_attention_layers_class[i](
                output_class,
                output,
                memory_mask=None,
                memory_key_padding_mask=None,  # here we do not apply masking on padded region
        ...
        out = {
            ...
            'pred_alignmasks': predictions_alignmask[-1],
            'aux_outputs': self._set_aux_loss(predictions_class if self.mask_classification else None, predictions_mask, predictions_alignmask)
        }
        return out
    ...
    def forward_prediction_heads(self, output, mask_features, attn_mask_target_size, output_class):
        ...
        outputs_alignweight = outputs_alignweight.softmax(-1)
        outputs_alignmask = torch.einsum("bql,blhw->bqhw", outputs_alignweight, outputs_mask)


        return outputs_class, outputs_mask, None, outputs_alignmask

    @torch.jit.unused
    def _set_aux_loss(self, outputs_class, outputs_seg_masks, outputs_alignmask):
        # this is a workaround to make torchscript happy, as torchscript
        # doesn't support dictionary with non-homogeneous values, such
        # as a dict having both a Tensor and a list.
        if self.mask_classification:
            return [{"pred_logits": a, "pred_masks": b, "pred_alignmasks" : c} for a, b, c in zip(outputs_class[:-1], outputs_seg_masks[:-1], outputs_alignmask[:-1])]
        else:
            return [{"pred_masks": b} for b in outputs_seg_masks[:-1]]


@TRANSFORMER_DECODER_REGISTRY.register()
class MultiScaleMaskedTransformerDecoderAlignmaskAttentionSigmoid(MultiScaleTransformerDecoderAlignmaskAttentionSigmoid):
    ...
    def forward_prediction_heads(self, output, mask_features, attn_mask_target_size, output_class):
        ...
        return outputs_class, outputs_mask, attn_mask, outputs_alignmask
