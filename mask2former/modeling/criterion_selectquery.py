...
class SetCriterion_SelectQuery(nn.Module):
    ...
    def __init__(self, num_classes, matcher, matcher_encoder, weight_dict, eos_coef, losses,
        ...
        self.matcher_encoder = matcher_encoder
    ...
    def loss_labels(self, outputs, targets, indices, num_masks, indices_encoder):
    ...
    def loss_encoder_labels(self, outputs, targets, indices, num_masks, indices_encoder):
        """Classification loss (NLL)
        targets dicts must contain the key "labels" containing a tensor of dim [nb_target_boxes]
        """
        assert "pred_logits" in outputs
        ...
        target_classes_o = torch.cat([t["labels"][J] for t, (_, J) in zip(targets, indices_encoder)])
        target_classes = torch.full(
            src_logits.shape[:2], self.num_classes, dtype=torch.int64, device=src_logits.device
        )
        target_classes[idx] = target_classes_o
    ...
    def loss_masks(self, outputs, targets, indices, num_masks, indices_encoder):
    ...
    def get_loss(self, loss, outputs, targets, indices, num_masks, indices_encoder):
        loss_map = {
            'labels': self.loss_labels,
            'masks': self.loss_masks,
            'encoderlabels': self.loss_encoder_labels,
        }
        assert loss in loss_map, f"do you really want to compute {loss} loss?"
        return loss_map[loss](outputs, targets, indices, num_masks, indices_encoder)
    ...
    def forward(self, outputs, targets):
        ...
        indices_encoder = self.matcher_encoder(outputs_without_aux, targets)
        ...
        for loss in self.losses:
            losses.update(self.get_loss(loss, outputs, targets, indices, num_masks, indices_encoder))
        ...
        if "aux_outputs" in outputs:
            for i, aux_outputs in enumerate(outputs["aux_outputs"]):
                ...
                for loss in self.losses:
                    if loss != "encoderlabels":
                        l_dict = self.get_loss(loss, aux_outputs, targets, indices, num_masks, indices_encoder)
    ...
    def __repr__(self):
        ...
        body = [
            ...
            "matcher_encoder: {}".format(self.matcher_encoder.__repr__(_repr_indent=8)),
