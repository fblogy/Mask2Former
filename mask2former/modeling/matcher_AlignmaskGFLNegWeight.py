...
class HungarianMatcher(nn.Module):
    ...
    def memory_efficient_forward(self, outputs, targets, use_ds=False):
        ...
        for b in range(bs):
            ...
            out_alignmask = outputs["pred_alignmasks"][b]
            # tmp_mask = out_mask.sigmoid().cpu().numpy()
            # gt masks are already padded when preparing target
            tgt_mask = targets[b]["masks"].to(out_mask)

            out_mask = out_mask[:, None]
            out_alignmask = out_alignmask[:, None]
            tgt_mask = tgt_mask[:, None]


            out_mask = out_mask.float()
            out_alignmask = out_alignmask.float()
            ...
            with autocast(enabled=False):
                ...
                cost_alignmask = batch_sigmoid_ce_loss_jit(out_alignmask, tgt_mask)

                # Compute the dice loss betwen masks
                cost_dice, iou = batch_dice_loss_jit(out_mask, tgt_mask)
                cost_aligndice, iou = batch_dice_loss_jit(out_alignmask, tgt_mask)
            ...
            # )
            C = (
                self.cost_mask * cost_alignmask
                + self.cost_class * cost_class
                + self.cost_dice * cost_aligndice
            )
            C_mask = (
                cost_mask + cost_dice 
            )
            C_mask = C_mask.reshape(num_queries, -1).cpu()
            indices_class.append(linear_sum_assignment(C_mask))
