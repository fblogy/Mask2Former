...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_masks, indices_class, log=True):
        ...
        src_pos_logits = outputs["pred_ious"].float()
        # target_classes_o = torch.cat([1 for t, (_, J) in zip(targets, indices)])
        target_classes = torch.full(
            src_logits.shape[:2], 0, dtype=torch.float, device=src_logits.device
        )
        target_classes[idx] = 1

        pos_weight = torch.zeros_like(target_classes, device=src_pos_logits.device)

        pos_weight += 10

        

        loss_iou = F.binary_cross_entropy_with_logits(src_pos_logits[:, :, 0], target_classes, pos_weight=pos_weight)

        losses = {"loss_ce": loss_ce, "loss_bestce": loss_bestce, "loss_iou" : loss_iou}
        if log:
    ...
    def loss_masks(self, outputs, targets, indices, num_masks, indices_class):
        ...
        src_iou = outputs["pred_ious"] #[B, N, 1]
        src_iou = src_iou[src_idx] # [match, 1]
        ...
        loss_dice = dice_loss_jit(point_logits, point_labels, src_iou, num_masks)
