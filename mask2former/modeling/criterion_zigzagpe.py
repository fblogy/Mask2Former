...
class SetCriterion(nn.Module):
    ...
    def loss_masks(self, outputs, targets, indices, num_masks, indices_class):
        ...
        src_masks_enhance = outputs["pred_masks_enhance"]
        src_masks = src_masks[src_idx]
        src_masks_enhance = src_masks_enhance[src_idx]
        ...
        src_masks_enhance = src_masks_enhance[:, None]
        target_masks = target_masks[:, None]
        src_masks = src_masks.float()
        src_masks_enhance = src_masks_enhance.float()
        ...
        with torch.no_grad():
            ...
            point_coords_enhance = get_uncertain_point_coords_with_randomness(
                src_masks_enhance,
                lambda logits: calculate_uncertainty(logits),
                self.num_points,
                self.oversample_ratio,
            ...
            # get gt labels
            point_labels_enhance = point_sample(
                target_masks,
                point_coords_enhance,
                align_corners=False,
            ).squeeze(1)
        ...
        point_logits_enhance = point_sample(
            src_masks_enhance,
            point_coords,
            align_corners=False,
        ).squeeze(1)
        ...
        losses = {
            ...
            "loss_mask_eh": sigmoid_ce_loss_jit(point_logits_enhance, point_labels_enhance, num_masks),
            "loss_dice_eh": dice_loss_jit(point_logits_enhance, point_labels_enhance, num_masks),
        }

        del src_masks
        del src_masks_enhance
        del target_masks
        return losses
