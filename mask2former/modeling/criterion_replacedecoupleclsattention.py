...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_masks, indices_decouple, log=True):
        """Classification loss (NLL)
        targets dicts must contain the key "labels" containing a tensor of dim [nb_target_boxes]
        """
        assert "pred_logits" in outputs
        assert "pred_decouple_logits" in outputs
        src_logits = outputs["pred_logits"].float() #[B, N, C]
        src_decouple_logits = outputs["pred_decouple_logits"].float()
        ...
        idx_decouple = self._get_src_permutation_idx(indices_decouple)
        ...
        target_classes_o_decouple = torch.cat([t["labels"][J] for t, (_, J) in zip(targets, indices_decouple)])
        ...
        target_classes_decouple = torch.full(src_decouple_logits.shape[:2], self.num_classes, dtype=torch.int64, device=src_logits.device)

        target_classes[idx] = target_classes_o
        target_classes_decouple[idx_decouple] = target_classes_o_decouple
        # target_classes[idx_loc] = target_classes_loc
        # print('target_classes', target_classes)

        loss_ce = F.cross_entropy(src_logits.transpose(1, 2), target_classes, self.empty_weight)
        loss_bestce = F.cross_entropy(src_decouple_logits.transpose(1, 2), target_classes_decouple, self.empty_weight)
        ...
        if log:
            ...
            losses['loss_class_error_best'] = 100 - accuracy(src_decouple_logits[idx_decouple], target_classes_o_decouple)[0]
        return losses
    ...
    @torch.no_grad()
    def loss_cardinality(self, outputs, targets, indices, num_boxes, indices_decouple):
        """ Compute the cardinality error, ie the absolute error in the number of predicted non-empty boxes
        This is not really a loss, it is intended for logging purposes only. It doesn't propagate gradients
        """
        pred_logits = outputs['pred_logits']
        pred_decouple_logits = outputs['pred_decouple_logits']
        device = pred_logits.device
        tgt_lengths = torch.as_tensor([len(v["labels"]) for v in targets], device=device)
        # Count the number of predictions that are NOT "no-object" (which is the last class)
        card_pred = (pred_logits.argmax(-1) != pred_logits.shape[-1] - 1).sum(1)
        card_err = F.l1_loss(card_pred.float(), tgt_lengths.float())
        card_pred_decouple = (pred_decouple_logits.argmax(-1) != pred_decouple_logits.shape[-1] - 1).sum(1)
        card_err_deocuple = F.l1_loss(card_pred_decouple.float(), tgt_lengths.float())
        losses = {'loss_cardinality_error': card_err, 'loss_cardinality_error_best': card_err_deocuple}
        return losses
