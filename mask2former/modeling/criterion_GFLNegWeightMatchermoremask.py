...
dice_loss_jit2 = torch.jit.script(dice_loss2)  # type: torch.jit.ScriptModule
...
sigmoid_ce_loss_jit2 = torch.jit.script(sigmoid_ce_loss2)  # type: torch.jit.ScriptModule
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_masks, indices_class, qiou, log=True):
        ...
        losses = {"loss_ce": loss_ce * 0, "loss_bestce": loss_bestce * 0}
        if log:
    ...
    def loss_masks(self, outputs, targets, indices, num_masks, indices_mask, qiou):
        ...
        src_idx = self._get_src_permutation_idx(indices_mask)
        tgt_idx = self._get_tgt_permutation_idx(indices_mask)    
        src_masks = outputs["pred_masks"]
        src_masks = src_masks[src_idx]
        masks = [t["masks"] for t in targets]
        # TODO use valid to mask invalid areas due to padding in loss
        ...
        losses = {
            "loss_mask": loss_mask + sigmoid_ce_loss_jit2(point_logits, point_labels, num_masks),
            "loss_dice": (loss_dice * iou).sum() + dice_loss_jit2(point_logits, point_labels, num_masks),
