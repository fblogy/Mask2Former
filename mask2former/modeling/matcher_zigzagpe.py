...
class HungarianMatcher(nn.Module):
    ...
    def memory_efficient_forward(self, outputs, targets):
        ...
        for b in range(bs):
            ...
            out_mask_enhance = outputs["pred_masks_enhance"][b]
            # tmp_mask = out_mask.sigmoid().cpu().numpy()
            # gt masks are already padded when preparing target
            tgt_mask = targets[b]["masks"].to(out_mask)

            out_mask = out_mask[:, None]
            out_mask_enhance = out_mask_enhance[:, None]
            tgt_mask = tgt_mask[:, None]

            out_mask = out_mask.float()
            out_mask_enhance = out_mask_enhance.float()
            ...
            with autocast(enabled=False):
                ...
                cost_mask_enhance = batch_sigmoid_ce_loss_jit(out_mask_enhance, tgt_mask)

                # Compute the dice loss betwen masks
                cost_dice = batch_dice_loss_jit(out_mask, tgt_mask)

                cost_dice_enhance =  batch_dice_loss_jit(out_mask_enhance, tgt_mask)
            # Final cost matrix
            ...
            # )
            C = (
                self.cost_mask * (cost_mask + cost_mask_enhance) / 2
                + self.cost_class * cost_class
                + self.cost_dice * (cost_dice + cost_dice_enhance) / 2
            )
            C_class = (
                cost_class
            )
