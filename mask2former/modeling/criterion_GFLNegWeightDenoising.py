...
def dice_loss_jit(
    ...
    # if qiou == None:
    #     print('iou_d', iou_d.shape)
    #     print('src_logits[a,b]', src_logits[a,b].shape)
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_masks, indices_DN, qiou, num_masks_DN, log=True):
        ...
        idx_class = self._get_src_permutation_idx(indices)
    ...
    def loss_masks(self, outputs, targets, indices, num_masks, indices_DN, qiou, num_masks_DN):
        ...
        target_masks = target_masks.to(src_masks) #[B, N, H, W]
        ...
        loss_dice_dn, loss_iou_dn, iou = dice_loss_jit(point_logits, point_labels, src_logits, target_classes, num_masks_DN, qiou = None)
        loss_mask_dn = (sigmoid_ce_loss_jit(point_logits, point_labels, num_masks_DN) * iou).sum()
        loss_dice_dn = (loss_dice_dn * iou).sum()
        loss_iou_dn = loss_iou_dn * src_logits.shape[1]
        ...
        losses = {
            ...
            "loss_dice": loss_dice,
            "loss_iou" : loss_iou,
            "loss_mask_dn" : loss_mask_dn,
            "loss_dice_dn" : loss_dice_dn,
            "loss_iou_dn" : loss_iou_dn,
        }
    ...
    @torch.no_grad()
    def loss_cardinality(self, outputs, targets, indices, num_boxes, indices_class, qiou, num_masks_DN):
    ...
    def get_loss(self, loss, outputs, targets, indices, num_masks, indices_class, qiou, num_masks_DN):
        loss_map = {
        ...
        return loss_map[loss](outputs, targets, indices, num_masks, indices_class, qiou, num_masks_DN)
    ...
    def forward(self, outputs, targets):
        ...
        # for i in range(10):
        indices, indices_DN, qiou = self.matcher(outputs_without_aux, targets)
        ...
        num_masks_DN = sum(t[0].numel() for t in indices_DN)
        num_masks_DN = torch.as_tensor(
            [num_masks_DN], dtype=torch.float, device=next(iter(outputs.values())).device
        )
        if is_dist_avail_and_initialized():
            torch.distributed.all_reduce(num_masks_DN)
        num_masks_DN = torch.clamp(num_masks_DN / get_world_size(), min=1).item()

        # Compute all the requested losses
        # print('Final')
        losses = {}
        for loss in self.losses:
            losses.update(self.get_loss(loss, outputs, targets, indices, num_masks, indices_DN, qiou, num_masks_DN))
        ...
        if "aux_outputs" in outputs:
            for i, aux_outputs in enumerate(outputs["aux_outputs"]):
                ...
                for loss in self.losses:
                    l_dict = self.get_loss(loss, aux_outputs, targets, indices, num_masks, indices_DN, qiou, num_masks_DN)
...
class SetCriterion_Decouple(nn.Module):
    ...
    def loss_masks(self, outputs, targets, indices, num_masks, loc_match_indices):
        ...
        # TODO use valid to mask invalid areas due to padding in loss
        target_masks, valid = nested_tensor_from_tensor_list(masks).decompose()  #[B, N, H, W]
        # print(target_masks.shape)
