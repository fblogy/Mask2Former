...
from .modeling.criterion_replacedecoupleclsattention import SetCriterion
from .modeling.matcher_replacedecoupleclsattention import HungarianMatcher
import time

@META_ARCH_REGISTRY.register()
class MaskFormerReplaceDecoupleClsAttention(nn.Module):
    """
    ...
    def from_config(cls, cfg):
        ...
        weight_dict = {"loss_ce": class_weight, "loss_bestce": class_weight, "loss_mask": mask_weight, "loss_dice": dice_weight, "loss_cardinality_error":0.0001, "loss_class_error":0.0001, "loss_cardinality_error_best":0.0001, "loss_class_error_best":0.0001}
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            mask_cls_results_decouple = outputs["pred_decouple_logits"]
            ...
            processed_results = []
            for mask_cls_result, mask_pred_result, mask_cls_result_decouple, input_per_image, image_size in zip(
                    mask_cls_results, mask_pred_results, mask_cls_results_decouple, batched_inputs, images.image_sizes):
                ...
                if self.instance_on:
                    instance_r = retry_if_cuda_oom(self.instance_inference)(mask_cls_result, mask_pred_result, mask_cls_result_decouple)
    ...
    def instance_inference(self, mask_cls, mask_pred, mask_cls_decouple):
        ...
        scores_decouple = F.softmax(mask_cls_decouple, dim=-1)[:, :-1] 
        ...
        # print(scores_decouple.shape)
        scores_decouple, sort_indices = torch.sort(scores_decouple, dim = 0, descending=True)
        # print(scores_decouple.shape)
        cls_cnt = torch.zeros(self.sem_seg_head.num_classes, dtype=torch.int64)
        

        scores_per_image, topk_indices = scores.flatten(0, 1).topk(self.test_topk_per_image, sorted=True)
        ...
        for i in range(scores_per_image.shape[0]):
            lb = labels_per_image[i]
            scores_per_image[i] = scores_decouple[cls_cnt[lb], lb]
            cls_cnt[lb] += 1
